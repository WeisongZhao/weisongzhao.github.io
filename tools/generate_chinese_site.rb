...
SEO = {
  ...
  "open-source.html" => {
    en_title: "Open-source Software | Weisong Zhao Lab",
    en_description: "Open-source imaging software from the Weisong Zhao Lab, including Sparse deconvolution, SACD, RIED, PANEL, and SN2N toolkits.",
    zh_title: "开源软件 | 赵唯淞课题组",
    zh_description: "赵唯淞课题组发布的开源成像软件，包括稀疏解卷积、SACD、RIED、PANEL和SN2N工具箱。"
  },
  "open-source-archive.html" => {
    en_title: "Open-source Archive | Weisong Zhao Lab",
    en_description: "Archived open-source microscopy and image-analysis utilities developed or maintained by the Weisong Zhao Lab.",
...
PROJECTS = {
  "SparseDecon" => [
    "稀疏解卷积",
    "<b>一种适用于荧光显微成像的通用后处理框架。</b>利用显微镜点扩散函数所诱导的稀疏性和连续性先验，我们开发了一种多约束解卷积算法；在低信噪比条件下，该算法可将分辨率提升至现有显微成像硬件物理极限的约两倍。该方法已在多种显微成像方式上完成测试：<br>1. 宽场显微成像；<br>2. TIRF显微成像；<br>3. 光片显微成像；<br>4. 共聚焦显微成像；<br>5. 多光子显微成像；<br>6. 结构光照明显微成像；<br>7. 基于涨落的超分辨显微成像；<br>8. 膨胀显微成像；<br>9. STED显微成像；<br>10. 用作单分子定位显微成像的预处理。<br>该方法适用于单层图像、时间序列和三维体数据。"
  ],
  "SACD" => [
    "基于涨落的超分辨成像",
  ...
  "SN2N" => [
    "无监督学习降噪",
    "在活细胞超分辨（SR）显微成像中，每一个采集到的光子都十分宝贵。我们开发了一种数据高效的深度学习降噪方案，用于改善多种SR成像方式。该方法SN2N是一个自启发Noise2Noise模块，包含自监督数据生成和自约束学习过程。SN2N的性能可媲美监督学习方法，同时无需大规模训练集和干净真值，仅使用单帧噪声图像即可训练。我们证明，SN2N可将光子利用效率提高一至两个数量级，并兼容多种成像方式，可用于三维、多色和时间序列SR显微成像。我们还将SN2N集成到不同的SR重建算法中，有效减轻图像伪影。我们预期SN2N将改善活细胞SR成像并推动后续进展。"
  ],
  "SparseSIM" => [
  ...
  "PANEL" => [
    "局部图像质量定量评估",
    "PANEL（逐像素误差位置分析）是一套在超分辨尺度上定量绘制重建质量的框架，可评估传统方法和深度学习方法。rFRC：滚动傅里叶环相关（rFRC）用于定量绘制局部图像质量，包括有效分辨率和数据不确定性。较低的有效分辨率意味着更高的误差存在概率，因此可用来表示揭示误差分布的不确定性。PANEL：我们还将滤波后的rFRC与截断的RSM（分辨率尺度误差图）结合，形成完整的PANEL图；当宽场参考图像不可用时，可关闭RSM这一可选功能。PANEL为生物学研究人员提供简洁可视化，用于定性定位可靠性较低的区域。"
  ],
  "informatics" => [
    "基于成像的生物医学信息学",
...
OPEN_SOURCE = {
  "Sparse-deconvolution" => "本仓库提供稀疏解卷积软件的更新版本。它是一种适用于荧光显微成像的通用后处理框架。<br><b>基本功能</b>：<br>1. 降噪；<br>2. 解卷积（三维分辨率提升）；<br>3. 上采样；<br>4. 去除背景（将落射荧光效果改善为类似TIRF）；<br>5. 去除伪影。",
  "SACD" => "1. SACD仅需20帧图像即可实现横向和轴向分辨率约两倍提升。<br>2. SACD无需额外光学器件，可为商用显微镜或自建系统灵活增加直接超分辨成像能力。<br>3. 稀疏解卷积辅助SACD（Sparse-SACD）可实现快速四维活细胞超分辨成像。",
  ...
  "PANEL" => "rFRC：滚动傅里叶环相关（rFRC）用于定量绘制局部图像质量，包括有效分辨率和数据不确定性。较低的有效分辨率意味着更高的误差存在概率，因此可用来表示揭示误差分布的不确定性。PANEL：我们还将滤波后的rFRC与截断的RSM（分辨率尺度误差图）结合，形成完整的PANEL（逐像素误差位置分析）图；当宽场参考图像不可用时，可关闭RSM这一可选功能。PANEL为生物学研究人员提供简洁可视化，用于定性定位可靠性较低的区域。",
  "SN2N" => "我们的SN2N（自启发Noise2Noise）性能可媲美监督学习方法，并且无需大规模数据集和干净真值。首先，我们利用超分辨图像的空间冗余构建自监督数据生成策略，通过对角重采样和傅里叶插值实现单帧Noise2Noise。其次，我们进一步引入自约束学习过程，以提高性能和数据效率。最后，我们提供Patch2Patch数据增强（在多个维度上进行随机图块变换），进一步提升数据效率。<br><b>基本功能</b>：<br>1. 无监督学习降噪；<br>2. 数据增强。",
...
def translate_common(text)
  replacements = {
    ...
    ">Open-source<" => ">开源<", ">Publications<" => ">论文<", ">Gallery<" => ">画廊<",
    ">Contact<" => ">联系<", ">Paper\n" => ">论文\n", ">Code\n" => ">代码\n",
    ">Preprint\n" => ">预印本\n", ">Software\n" => ">软件\n", ">Instrument\n" => ">仪器\n",
...
def translate_home(text)
  replacements = {
    "Tailored Microscopes for Biomedical Research Across Scales" => "面向跨尺度生物医学研究的定制显微成像",
    ...
    "Smart Bioimage Analysis &amp; Biomedical Devices" => "智能生物图像分析与生物医学装置",
    "We focus on developing advanced biomedical imaging technologies across modalities. Our research spans new imaging physics, bottom-up system design, and the co-design of hardware and computational algorithms tailored to specific imaging scenarios. We aim to optimize spatial, temporal, and information resolution under practical constraints such as photon or signal budget, acquisition speed, imaging depth, and system complexity." => "我们专注于发展跨模态的先进生物医学成像技术。研究涵盖新型成像物理、自底向上的系统设计，以及针对具体成像场景的硬件与计算算法协同设计。我们的目标是在光子或信号预算、采集速度、成像深度和系统复杂度等实际约束下，优化空间分辨率、时间分辨率和信息分辨率。",
    "is a physics-informed reconstruction framework that exploits sparsity and structural continuity to enhance resolution, contrast, and SNR beyond the limits of microscopy, particularly under low-SNR conditions. Applications include <a href=\"projects.html#SparseSIM\">Sparse-SIM</a> and <a href=\"projects.html#SparseSDSIM\">Sparse SD-SIM</a>." => "是一种物理信息驱动的重建框架，利用稀疏性和结构连续性，在超越显微成像限制的同时提高分辨率、对比度和信噪比，尤其适用于低信噪比条件。应用包括<a href=\"projects.html#SparseSIM\">Sparse-SIM</a>和<a href=\"projects.html#SparseSDSIM\">Sparse SD-SIM</a>。",
    ...
    "We develop machine-learning methods for the reconstruction and analysis of multidimensional biomedical and natural signals. We exploit both explicit physical and structural priors to design interpretable computational models, and data-driven priors to develop deep-learning approaches for problems that are difficult to address with conventional methods. Our work includes self-supervised learning, representation learning, Bayesian learning, and foundation models, with applications ranging from reducing the photon, acquisition-time, and hardware requirements of microscopy to accelerating biological image reconstruction, profiling, and discovery." => "我们发展用于重建和分析多维生物医学信号与自然信号的机器学习方法。我们既利用显式物理与结构先验设计可解释计算模型，也利用数据驱动先验，为传统方法难以解决的问题发展深度学习方案。研究包括自监督学习、表征学习、贝叶斯学习和基础模型，应用范围涵盖降低显微成像的光子、采集时间和硬件需求，以及加速生物图像重建、表型分析和发现。",
    "(Self-inspired Noise2Noise) is a self-supervised learning-to-denoise framework that achieves performance competitive with supervised learning while eliminating the need for large paired datasets and clean ground-truth images. It enables robust denoising across diverse biomedical imaging modalities and supports photon-efficient, long-term, and low-phototoxicity imaging." => "（自启发Noise2Noise）是一种自监督学习降噪框架，性能可媲美监督学习，同时无需大规模配对数据集和干净真值图像。它可在多种生物医学成像模态中实现稳健降噪，并支持光子高效、长时程和低光毒成像。",
    ...
    "From imaging physics and instrumentation to computational reconstruction and system–algorithm co-design." => "从成像物理与仪器，到计算重建及系统—算法协同设计。",
  ...
  news = <<~HTML
    ...
    <div class="card-body">
      <div class="card-text">
        ...
        <b>2026-04：</b> 祝贺课题组博士生苏德尔。两项推进光场内窥探头的研究分别发表于<a href="https://www.sciencedirect.com/science/article/pii/S0143816626002605">Optics and Lasers in Engineering</a>和<a href="https://opg.optica.org/oe/fulltext.cfm?uri=oe-34-8-15664">Optics Express</a>。<br><br>
        <b>2026-02：</b> 祝贺课题组博士生桂家辉。<a href="https://www.biorxiv.org/content/10.64898/2026.02.04.703714v1">RIED</a>以bioRxiv预印本形式发布，同时开放<a href="https://github.com/SR-Wiki/RIEDm">MATLAB</a>工具箱。<hr>
        <b>2025-09：</b> 课题组PI晋升为哈尔滨工业大学<a href="http://homepage.hit.edu.cn/weisongzhao">长聘教授</a>。<br><br>
        <b>2025-09：</b> 祝贺课题组合作研究人员曲丽颖。关于SN2N的述评发表于<a href="https://onlinelibrary.wiley.com/doi/10.1002/ctm2.70390">Clinical and Translational Medicine</a>。<br><br>
        ...
        <b>2025-07：</b> <a href="https://github.com/SR-Wiki/">SR-Wiki</a>现为课题组发布最新成像与分析工具的官方GitHub组织，并提供课题组成像工具的实现。<br><br>
        ...
        <b>2024-06：</b> 祝贺课题组博士生曲丽颖和黄园园。<a href="https://doi.org/10.1101/2024.01.23.576521">SN2N</a>已被<i>Nature Methods</i>接收发表。<br><br>
        <b>2024-01：</b> 祝贺课题组博士生曲丽颖和黄园园。<a href="https://doi.org/10.1101/2024.01.23.576521">SN2N</a>以bioRxiv预印本形式发布（投稿<i>Nature Methods</i>审稿中），同时开放<a href="https://github.com/WeisongZhao/SN2N">Python</a>工具箱。<br><br>
...
def translate_team(text)
  text.sub!(%r{(<h5[^>]*>Weisong Zhao \(赵唯淞\), Ph\.D\..*?</h5>).*?<p>.*?</p>}m,
            "\\1\n<p>赵唯淞，哈尔滨工业大学仪器科学与工程学院长聘教授、博士生导师、青年科学家工作室负责人，主要从事生物医学显微成像技术研究。分别于2017年和2022年在哈尔滨工业大学物理系、仪器科学与工程学院获得理学学士和工学博士学位。近年来以第一/共同第一作者或通讯/共同通讯作者身份在Nature、Nature Biotechnology、Nature Photonics、Nature Methods等期刊发表学术论文。参加国内外学术会议大会报告、主题报告和邀请报告五十余次，已授权或申请PCT国际发明专利、中国发明专利二十余项。</p>")
...
def translate_open_source(text)
  text.gsub!("represents the Weisong Zhao Lab’s imaging toolkit implementations.", "是赵唯淞课题组成像工具箱的官方实现平台。")
...
def translate_contact(text)
  ...
  text.gsub!("<b>Room J714</b>, Technology Innovation Building (TIB), Harbin Institute of Technology, Harbin, China.",
             "<b>J714室</b>，哈尔滨工业大学科学园科技创新大厦（TIB），中国哈尔滨。")
  text
end
